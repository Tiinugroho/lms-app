    ...
    <main class="flex-1 pb-12 pt-8 relative w-full">
        ...
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 w-full">
            <div class="mb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div>
                    <h2 class="text-2xl font-bold text-gray-900">Tambah Jadwal Pelajaran</h2>
                    <p class="text-sm text-gray-500 mt-1">Sistem akan otomatis mengecek jika terjadi tabrakan jadwal.</p>
                </div>
            ...
            @error('clash')
                <div class="mb-6 bg-red-50 border-l-4 border-red-500 p-4 rounded-r-md shadow-sm flex items-start animate-bounce-short">
                    ...
                    <div>
                        <h3 class="text-sm font-bold text-red-800">Gagal Menyimpan! Terdeteksi Tabrakan Jadwal</h3>
            ...
            <form action="{{ route('admin.schedules.store') }}" method="POST" class="bg-white shadow-sm sm:rounded-xl border border-gray-200 overflow-hidden">
                @csrf
                ...
                <div class="p-6 sm:p-10 space-y-8">
                    ...
                    <div class="bg-indigo-50 border border-indigo-100 rounded-lg p-4 flex items-center justify-between">
                        <div>
                            <p class="text-xs font-bold text-indigo-800 uppercase tracking-wider">Tahun Ajaran Aktif</p>
                            <p class="text-lg font-bold text-indigo-900 mt-0.5">{{ $activeYear->period }} - Semester {{ $activeYear->semester }}</p>
                        </div>
                        <svg class="w-8 h-8 text-indigo-200" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                    </div>
                    ...
                    <div class="grid grid-cols-12 gap-6">
                        ...
                        <div class="col-span-12 md:col-span-3">
                            ...
                            <input type="time" name="start_time" value="{{ old('start_time') }}" required class="block w-full rounded-md border border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-base py-2.5 px-3">
                        </div>

                        <div class="col-span-12 md:col-span-3">
                            <label class="block text-sm font-semibold text-gray-700 mb-2">Jam Selesai <span class="text-red-500">*</span></label>
                            <input type="time" name="end_time" value="{{ old('end_time') }}" required class="block w-full rounded-md border border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-base py-2.5 px-3">
                        </div>
