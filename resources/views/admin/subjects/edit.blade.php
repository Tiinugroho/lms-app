    ...
    <main class="flex-1 pb-12 pt-8 relative">
        ...
        <div class="mx-auto max-w-12xl px-4 sm:px-6 lg:px-8 w-full">
            <div class="mb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div>
                    <h2 class="text-2xl font-bold text-gray-900">Edit Mata Pelajaran</h2>
                    <p class="text-sm text-gray-500 mt-1">Perbarui informasi nama atau kode mata pelajaran.</p>
                </div>
            ...
            <form action="{{ route('admin.subjects.update', $subject->id) }}" method="POST" class="bg-white shadow-sm sm:rounded-xl border border-gray-200 overflow-hidden">
                @csrf
                @method('PUT')
                <div class="p-6 sm:p-10">
                    ...
                    <div class="grid grid-cols-12 gap-6">
                        <div class="col-span-12 md:col-span-4">
                            ...
                            <input type="text" name="code" value="{{ old('code', $subject->code) }}" required class="block w-full rounded-md border border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-base py-2.5 px-3 transition">
                        </div>
                        <div class="col-span-12 md:col-span-8">
                            <label class="block text-sm font-medium text-gray-700 mb-2">Nama Mata Pelajaran <span class="text-red-500">*</span></label>
                            <input type="text" name="name" value="{{ old('name', $subject->name) }}" required class="block w-full rounded-md border border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-base py-2.5 px-3 transition">
                        </div>
                        <div class="col-span-12">
                            <label class="block text-sm font-medium text-gray-700 mb-2">Deskripsi (Opsional)</label>
                            <textarea name="description" rows="4" class="block w-full rounded-md border border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-base py-2.5 px-3 transition">{{ old('description', $subject->description) }}</textarea>
                        </div>
                    </div>
                </div>
