        ...
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 w-full">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
                <div>
                    ...
                    <p class="text-sm text-gray-500 mt-1">Kelola data profil, NIK/NISN, akun login, dan cetak ID Card peserta didik.</p>
                </div>
                <a href="{{ route('admin.students.create') }}" class="w-full sm:w-auto inline-flex items-center justify-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 shadow-sm transition">
                    + Tambah Siswa
                </a>
            </div>
                ...
                <div class="bg-white/60 backdrop-blur-md border border-white p-5 sm:p-6 rounded-xl shadow-sm transition hover:shadow-md">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <h3 class="mt-1 text-2xl sm:text-3xl font-bold text-gray-900">{{ $totalStudents }}</h3>
                        </div>
                ...
                <div class="bg-white/60 backdrop-blur-md border border-white p-5 sm:p-6 rounded-xl shadow-sm transition hover:shadow-md">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <h3 class="mt-1 text-2xl sm:text-3xl font-bold text-gray-900">{{ $activeStudents }}</h3>
                        </div>
                ...
                <div class="bg-white/60 backdrop-blur-md border border-white p-5 sm:p-6 rounded-xl shadow-sm transition hover:shadow-md">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <h3 class="mt-1 text-2xl sm:text-3xl font-bold text-gray-900">{{ $inactiveStudents }}</h3>
                        </div>
            ...
            <div class="bg-white shadow-sm sm:rounded-lg border border-gray-100 p-4 sm:p-6 w-full relative">
                @if ($students->count() > 0)
                    <div class="overflow-x-auto w-full">
                        <table id="studentTable" class="w-full min-w-[900px] text-left border-collapse">
                            <thead>
                                <tr class="border-b border-gray-200 text-sm font-semibold text-gray-600 bg-gray-50/50">
                                    ...
                                    <th class="px-4 py-3 whitespace-nowrap">Profil Siswa</th>
                                    <th class="px-4 py-3 whitespace-nowrap">NIS / NISN / NIK</th>
                                    <th class="px-4 py-3 whitespace-nowrap">L/P</th>
                                    ...
                                    <tr class="hover:bg-gray-50 transition">
                                        ...
                                        <td class="px-4 py-4">
                                            <div class="flex items-center gap-3">
                                                <img src="{{ $student->user->avatar_url }}" alt="Avatar" class="h-10 w-10 rounded-full object-cover border border-gray-200 shadow-sm">
                                                <div>
                                                    <div class="font-medium text-gray-900 whitespace-nowrap">{{ $student->user->name }}</div>
                                                    <div class="text-sm text-gray-500 whitespace-nowrap">{{ $student->user->email }}</div>
                                                </div>
                                            </div>
                                        </td>

                                        <td class="px-4 py-4 text-sm text-gray-600 whitespace-nowrap">
                                            <span class="font-semibold text-indigo-600">{{ $student->nis ?? '-' }}</span> <br>
                                            <span class="text-xs text-gray-500">NISN: {{ $student->nisn ?? '-' }} | NIK: {{ $student->nik ?? '-' }}</span>
                                        </td>

                                        <td class="px-4 py-4 text-sm text-gray-600 whitespace-nowrap">{{ $student->gender }}</td>
                                        ...
                                        <td class="px-4 py-4 whitespace-nowrap">
                                            <div class="flex items-center justify-end gap-2">
                                                ...
                                                <a href="{{ route('admin.students.id-card', $student->id) }}" target="_blank" class="flex items-center gap-1 px-3 py-1.5 bg-emerald-100 text-emerald-700 rounded hover:bg-emerald-200 text-sm font-medium transition" title="Cetak ID Card">
                                                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z" /></svg>
                                                    Cetak
                                                </a>

                                                <a href="{{ route('admin.students.edit', $student->id) }}" class="flex items-center gap-1 px-3 py-1.5 bg-indigo-100 text-indigo-600 rounded hover:bg-indigo-200 text-sm font-medium transition">
                ...
                @else
                    <div class="flex flex-col items-center justify-center py-12 px-4 text-center">
                        ...
                        <p class="mt-1 text-sm text-gray-500 max-w-sm mx-auto">Sistem belum memiliki data peserta didik. Silakan tambahkan data baru.</p>
                        <div class="mt-6">
                            <a href="{{ route('admin.students.create') }}" class="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700">Tambah Siswa Pertama</a>
                        </div>
                    </div>
                @endif
            </div>
                    ...
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div class="sm:flex sm:items-start">
                            ...
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                ...
                                <div class="mt-2">
                                    <p class="text-sm text-gray-500">Apakah Anda yakin ingin menghapus data siswa ini? Tindakan ini bersifat permanen.</p>
                                </div>
                            </div>
                        </div>
                    </div>
